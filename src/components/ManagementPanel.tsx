...
const ManagementPanel = ({ state, onTakeAction, actionsEnabled = true }: ManagementPanelProps) => {
  ...
  const shouldOpenActions = actionsEnabled && ap.apRemaining > 0;
  const shouldOpenProjects = state.projects.some(p => p.status === 'active' || p.status === 'paused');

  const defaultOpen = [
    ...(shouldOpenActions ? ['actions'] : []),
    ...(shouldOpenProjects ? ['projects'] : []),
  ];
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Tip
            ...
            content={'AP is spent on management actions and projects in the Hall. It resets when you End Turn.'}
          />
        </div>
      </div>
      ...
      <div className="parchment-border rounded-sm bg-card p-3">
        <div className="grid grid-cols-4 gap-2">
          {(['coin', 'influence', 'supplies', 'intel'] as const).map(k => (
            <div key={k} className="rounded-sm bg-secondary/30 px-2 py-1">
      ...
      <Accordion type="multiple" defaultValue={defaultOpen} className="parchment-border rounded-sm bg-card">
        <AccordionItem value="actions" className="border-border">
          <AccordionTrigger className="px-4 text-xs font-display tracking-[0.25em] text-muted-foreground uppercase hover:no-underline">
            <div className="flex w-full items-center justify-between">
              <span>Actions</span>
              <span className="text-[10px] tracking-[0.22em] text-muted-foreground/70">
                {actionsEnabled ? `${availableActions.length} available` : 'Hall only'}
              </span>
            </div>
          </AccordionTrigger>
          <AccordionContent>
            <div className="px-4 pb-4">
              {!actionsEnabled && (
                <div className="mb-3 text-xs text-muted-foreground">
                  Return to Concord Hall to take actions.
                </div>
              )}
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="mt-2 text-[11px] text-muted-foreground">
                            ...
                            {action.oncePerChapter ? ` · once/chap` : ''}
                            <span className="ml-2 inline-flex align-middle">
                              <Tip
                                id="action-costs"
                                label="Tip: Action costs"
                                content={'Actions spend AP and sometimes resources. Cooldowns prevent repeating an action for several turns; some actions are limited to once per chapter.'}
                                kind="tooltip"
                              />
                            </span>
                          </div>
        ...
        <AccordionItem value="projects" className="border-border">
          <AccordionTrigger className="px-4 text-xs font-display tracking-[0.25em] text-muted-foreground uppercase hover:no-underline">
            <div className="flex w-full items-center justify-between">
              <span>Projects</span>
              <span className="text-[10px] tracking-[0.22em] text-muted-foreground/70">
                {state.projects.length ? `${state.projects.length} running` : '—'}
              </span>
            </div>
          </AccordionTrigger>
          <AccordionContent>
            <div className="px-4 pb-4">
              <div className="mb-2 flex items-center gap-2 text-[11px] text-muted-foreground">
                <span className="font-display tracking-[0.18em] uppercase">Tip</span>
                <Tip
                  id="projects"
                  label="Tip: Projects"
                  content={'Projects advance when you End Turn. Accelerate spends AP/resources to reduce remaining turns.'}
                  kind="popover"
                />
              </div>
        ...
        <AccordionItem value="resources" className="border-b-0 border-border">
          <AccordionTrigger className="px-4 text-xs font-display tracking-[0.25em] text-muted-foreground uppercase hover:no-underline">
            Resources
          </AccordionTrigger>
          <AccordionContent>
